...
class LocalStorage:
    ...
    def __init__(self):
        self._path = os.path.join(os.path.expanduser("~"), ".astra", "modules_cache")
        self._ensure_dirs()
    ...
    async def fetch(self, url: str, auth: typing.Optional[str] = None) -> str:
        ...
        try:
            r = await utils.run_sync(
                ...
                headers={
                    "User-Agent": "astra Userbot",
                    "X-astra-Version": ".".join(map(str, __version__)),
                    "X-astra-Commit-SHA": utils.get_git_hash(),
                    "X-astra-User": str(self._client.tg_id),
                },
            )
            r.raise_for_status()
        except Exception:
