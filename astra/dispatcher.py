...
from astratl.errors import FloodWaitError, RPCError
...
class CommandDispatcher:
    def __init__(
        ...
        self._me = self._client.astra_me.id
        self._cached_usernames = [
            (
                self._client.astra_me.username.lower()
                if self._client.astra_me.username
                else str(self._client.astra_me.id)
            )
        ]

        self._cached_usernames.extend(
            u.username.lower()
            for u in getattr(self._client.astra_me, "usernames", [])
            or []
        )
            ...
            with contextlib.suppress(Exception):
                self._external_bl = (
                    await utils.run_sync(
                        requests.get,
                        "https://ubguard.astra.life/blacklist.json",
                    )
                ).json()["blacklist"]
