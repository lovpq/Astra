...
from astratl.tl.tlobject import TLObject
...
def patched_import(name: str, *args, **kwargs):
    if name.startswith("telethon"):
        return native_import("astratl" + name[8:], *args, **kwargs)
    elif name.startswith("hikkatl"):
        return native_import("astratl" + name[7:], *args, **kwargs)
    elif name.startswith ("hikka"):
        return native_import("astra" + name[5:], *args, **kwargs)
...
class InfiniteLoop:
    ...
    def stop(self, *args, **kwargs):
        with contextlib.suppress(AttributeError):
            _astra_client_id_logging_tag = copy.copy(  # noqa: F841
...
def tag(*tags, **kwarg_tags):
    ...
    @loader.tag("only_messages", "only_pm", regex=r"^[.] ?astra$", from_id=659800858)
...
class Modules:
    ...
    def __init__(
        ...
        self.client.astra_inline = self.inline
    ...
    async def _junk_collector(self):
        ...
        while True:
            ...
            for module in self.modules:
                commands.update(module.astra_commands)
                inline_handlers.update(module.astra_inline_handlers)
                callback_handlers.update(module.astra_callback_handlers)
                watchers.extend(module.astra_watchers.values())
    ...
    def register_commands(self, instance: Module):
        ...
        if instance.__origin__.startswith("<core"):
            self._core_commands += list(
                map(lambda x: x.lower(), list(instance.astra_commands))
            )

        for _command, cmd in instance.astra_commands.items():
        ...
        for alias, cmd in self.aliases.copy().items():
            if cmd in instance.astra_commands:
                self.add_alias(alias, cmd)

        self.register_inline_stuff(instance)

    def register_inline_stuff(self, instance: Module):
        for name, func in instance.astra_inline_handlers.copy().items():
        ...
        for name, func in instance.astra_callback_handlers.copy().items():
    ...
    def register_watchers(self, instance: Module):
        ...
        for _watcher in instance.astra_watchers.values():
