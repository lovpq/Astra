...
from astratl import TelegramClient
from astratl import __name__ as __base_name__
from astratl import helpers
from astratl._updates import ChannelState, Entity, EntityType, SessionState
from astratl.errors import RPCError
from astratl.errors.rpcerrorlist import TopicDeletedError
from astratl.hints import EntityLike
from astratl.network import MTProtoSender
from astratl.tl import functions
from astratl.tl.alltlobjects import LAYER
from astratl.tl.functions.channels import GetFullChannelRequest
from astratl.tl.functions.users import GetFullUserRequest
from astratl.tl.tlobject import TLRequest
from astratl.tl.types import (
    ChannelFull,
    Message,
    Pong,
    Updates,
...
from astratl.utils import is_list_like
...
class CustomTelegramClient(TelegramClient):
    def __init__(self, *args, **kwargs):
        ...
        self._astra_entity_cache: typing.Dict[
            typing.Union[str, int],
            CacheRecordEntity,
        ] = {}

        self._astra_perms_cache: typing.Dict[
            typing.Union[str, int],
            CacheRecordPerms,
        ] = {}

        self._astra_fullchannel_cache: typing.Dict[
            typing.Union[str, int],
            CacheRecordFullChannel,
        ] = {}

        self._astra_fulluser_cache: typing.Dict[
            typing.Union[str, int],
            CacheRecordFullUser,
        ] = {}
    ...
    @property
    def astra_entity_cache(self) -> typing.Dict[int, CacheRecordEntity]:
        return self._astra_entity_cache

    @property
    def astra_perms_cache(self) -> typing.Dict[int, CacheRecordPerms]:
        return self._astra_perms_cache

    @property
    def astra_fullchannel_cache(self) -> typing.Dict[int, CacheRecordFullChannel]:
        return self._astra_fullchannel_cache

    @property
    def astra_fulluser_cache(self) -> typing.Dict[int, CacheRecordFullUser]:
        return self._astra_fulluser_cache
    ...
    async def get_entity(
        ...
        _astra_client_id_logging_tag = copy.copy(self.tg_id)  # noqa: F841
        ...
        if (
            not force
            and hashable_entity
            and hashable_entity in self._astra_entity_cache
            and (
                not exp
                or self._astra_entity_cache[hashable_entity].ts + exp > time.time()
            )
        ):
            logger.debug(
                "Using cached entity %s (%s)",
                entity,
                type(self._astra_entity_cache[hashable_entity].entity).__name__,
            )
            return copy.deepcopy(self._astra_entity_cache[hashable_entity].entity)

        resolved_entity = await super().get_entity(entity)

        if resolved_entity:
            cache_record = CacheRecordEntity(hashable_entity, resolved_entity, exp)
            self._astra_entity_cache[hashable_entity] = cache_record
            ...
            if getattr(resolved_entity, "id", None):
                ...
                self._astra_entity_cache[resolved_entity.id] = cache_record
            ...
            if getattr(resolved_entity, "username", None):
                ...
                self._astra_entity_cache[f"@{resolved_entity.username}"] = cache_record
                self._astra_entity_cache[resolved_entity.username] = cache_record
    ...
    async def get_perms_cached(
        ...
        if (
            ...
            and hashable_entity
            and hashable_user
            and hashable_user in self._astra_perms_cache.get(hashable_entity, {})
            and (
                not exp
                or self._astra_perms_cache[hashable_entity][hashable_user].ts + exp
                > time.time()
            )
        ):
            logger.debug("Using cached perms %s (%s)", hashable_entity, hashable_user)
            return copy.deepcopy(
                self._astra_perms_cache[hashable_entity][hashable_user].perms
            )
        ...
        if resolved_perms:
            ...
            self._astra_perms_cache.setdefault(hashable_entity, {})[hashable_user] = (
                cache_record
            )
            ...
            def save_user(key: typing.Union[str, int]):
                ...
                if getattr(user, "id", None):
                    self._astra_perms_cache.setdefault(key, {})[user.id] = cache_record

                if getattr(user, "username", None):
                    self._astra_perms_cache.setdefault(key, {})[f"@{user.username}"] = (
                        cache_record
                    )
                    self._astra_perms_cache.setdefault(key, {})[user.username] = (
                        cache_record
                    )
    ...
    async def get_fullchannel(
        ...
        if (
            not force
            and self._astra_fullchannel_cache.get(hashable_entity)
            and not self._astra_fullchannel_cache[hashable_entity].expired
            and self._astra_fullchannel_cache[hashable_entity].ts + exp > time.time()
        ):
            return self._astra_fullchannel_cache[hashable_entity].full_channel

        result = await self(GetFullChannelRequest(channel=entity))
        self._astra_fullchannel_cache[hashable_entity] = CacheRecordFullChannel(
            hashable_entity,
            result,
            exp,
        )
    ...
    async def get_fulluser(
        ...
        if (
            not force
            and self._astra_fulluser_cache.get(hashable_entity)
            and not self._astra_fulluser_cache[hashable_entity].expired
            and self._astra_fulluser_cache[hashable_entity].ts + exp > time.time()
        ):
            return self._astra_fulluser_cache[hashable_entity].full_user

        result = await self(GetFullUserRequest(entity))
        self._astra_fulluser_cache[hashable_entity] = CacheRecordFullUser(
            hashable_entity,
            result,
            exp,
        )
    ...
    async def _call(
        ...
        # list and you will be banned from astra federation.
