...
from astratl.extensions.html import CUSTOM_EMOJIS
from astratl.tl.functions.messages import (
    GetDialogFiltersRequest,
    UpdateDialogFilterRequest,
)
from astratl.tl.types import DialogFilter, Message
...
class UpdaterMod(loader.Module):
    """Updates itself, tracks latest astra releases, and notifies you, if update is required"""
    ...
    async def update_call(self, call: InlineCall):
        ...
        if call.data not in {"astra/update", "astra/ignore_upd"}:
            return

        if call.data == "astra/ignore_upd":
    ...
    async def changelog(self, message: Message):
        ...
        if (await self._client.get_me()).premium:
            changelog.replace('🌑 astra', '<emoji document_id=5192765204898783881>🌘</emoji><emoji document_id=5195311729663286630>🌘</emoji><emoji document_id=5195045669324201904>🌘</emoji>')
    ...
    async def restart_common(
        ...
        msg_obj = await utils.answer(
            ...
            self.strings("restarting_caption").format(
                ...
                if self._client.astra_me.premium
                and CUSTOM_EMOJIS
    ...
    async def client_ready(self):
        ...
        self._markup = lambda: self.inline.generate_markup(
            [
                {"text": self.strings("update"), "data": "astra/update"},
                {"text": self.strings("ignore"), "data": "astra/ignore_upd"},
            ]
        )
    ...
    async def _add_folder(self):
        ...
        if any(getattr(folder, "title", None) == "astra" for folder in folders.filters):
            return
        ...
        try:
            ...
            logger.critical(
                "Can't create astra folder. Possible reasons are:\n"
