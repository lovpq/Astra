...
import astratl
from astratl.errors.rpcerrorlist import MessageIdInvalidError
from astratl.sessions import StringSession
...
class Evaluator(loader.Module):
    ...
    async def e(self, message: Message):
        try:
            ...
            item = astraException.from_exc_info(*sys.exc_info())
    ...
    def censor(self, ret: str) -> str:
        ret = ret.replace(str(self._client.astra_me.phone), "&lt;phone&gt;")
        ...
        if btoken := self._db.get("astra.inline", "bot_token", False):
            ret = ret.replace(
                btoken,
                f'{btoken.split(":")[0]}:{"*" * 26}',
            )
    ...
    async def getattrs(self, message: Message) -> dict:
        ...
        return {
            ...
            "reply": reply,
            "r": reply,
            **self.get_sub(astratl.tl.types),
            **self.get_sub(astratl.tl.functions),
            "event": message,
            "chat": message.to_id,
            "astratl": astratl,
            "telethon": astratl,
                ...
                itertools.chain.from_iterable(
                    [
                        ...
                        for y in filter(
                            ...
                            and x[1] != obj
                            and x[1].__package__.rsplit(".", _depth)[0] == "astratl.tl",
                            obj.__dict__.items(),
                        )
                    ]
                )
