...
@loader.tds
class astraInfoMod(loader.Module):
    """Show userbot info"""

    strings = {"name": "astraInfo"}
    ...
    def __init__(self):
        self.config = loader.ModuleConfig(
            ...
            loader.ConfigValue(
                "banner_url",
                "https://i.imgur.com/YJA0ur8.jpeg",
    ...
    def _render_info(self, start: float) -> str:
        ...
        me = self.config['imgSettings'][0] if (self.config['imgSettings'][0] != "Лапокапканот") and self.config['switchInfo'] else '<b><a href="tg://user?id={}">{}</a></b>'.format(
            self._client.astra_me.id,
            utils.escape_html(get_display_name(self._client.astra_me)),
        ).replace('{', '').replace('}', '')
        ...
        return (
            ...
            if self.config["custom_message"]
                ...
                (
                    ...
                    if self._client.astra_me.premium and self.config["show_astra"]
                    else ""
                ),
    ...
    @loader.command()
    async def astrainfo(self, message: Message):
