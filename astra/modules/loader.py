...
import requests
from astratl.errors.rpcerrorlist import MediaCaptionTooLongError
from astratl.tl.functions.channels import JoinChannelRequest
from astratl.tl.types import Channel, Message, PeerUser
from astratl.errors.common import ScamDetectionError
...
class LoaderMod(loader.Module):
    ...
    async def approve_internal(
        ...
        await call.edit(
            ...
            photo="https://i.imgur.com/WgHoQAU.jpeg",
        )
    ...
    async def load_module(
        ...
        if re.search(r"# ?scope: ?astra_min", doc):
            ver = re.search(r"# ?scope: ?astra_min ((?:\d+\.){2}\d+)", doc).group(1)
        ...
        module_name = f"astra.modules.{uid}"
        doc = geek.compat(doc)
        ...
        try:
            try:
                ...
                async def inner_proxy():
                    ...
                    while True:
                        if hasattr(instance, "astra_wait_channel_approve"):
                            if message:
                                (
                                    module,
                                    channel,
                                    reason,
                                ) = instance.astra_wait_channel_approve
                                message = await utils.answer(
                                    message,
        ...
        try:
            developer_entity = await (
                ...
                if (
                    developer in self._client.astra_entity_cache
                    and getattr(
