...
class CoreMod(loader.Module):
    ...
    def __init__(self):
        self.config = loader.ModuleConfig(
            ...
            loader.ConfigValue(
                ...
                False,
                (
                    "Allow astra.t.me to control the actions of your userbot"
    ...
    @loader.command(ru_doc="Информация о Астра", en_doc="Information of Astra", ua_doc="Інформація про Астра", de_doc="Informationen über Astra")
    async def astracmd(self, message: Message):
        await utils.answer_file(
            message,
            "https://i.imgur.com/SpE3R2S.jpeg",
            self.strings("astra").format(
                (
                    utils.get_platform_emoji()
                    if self._client.astra_me.premium and CUSTOM_EMOJIS
                    else "⭐ <b>Astra userbot</b>"
                ),
                *version.__version__,
                utils.get_commit_url(),
                f"{astratl.__version__} #{astratl.tl.alltlobjects.LAYER}",
            )
            + (
                ""
    ...
    async def installationcmd(self, message: Message):
        ...
        if (not args or args not in {'-v', '-r', '-jh', '-ms', '-u'}) and \
            not (await self.inline.form(
                ...
                photo="https://i.imgur.com/Vuf26eL.jpeg",
            ...
            await self.client.send_file(
                message.peer_id,
                "https://i.imgur.com/Vuf26eL.jpeg",
