...
import grapheme
import astratl
import astratl.extensions
import astratl.extensions.html
import requests
from aiogram.types import Message as AiogramMessage
from astratl import hints
from astratl.tl.custom.message import Message
from astratl.tl.functions.account import UpdateNotifySettingsRequest
from astratl.tl.functions.channels import (
...
from .types import astraReplyMarkup, ListLike, Module
...
parser = astratl.utils.sanitize_parse_mode("html")
...
def get_chat_id(message: typing.Union[Message, AiogramMessage]) -> int:
    ...
    :return: Chat ID
    """
    return astratl.utils.resolve_id(
...
def get_entity_id(entity: hints.Entity) -> int:
    ...
    :return: Entity ID
    """
    return astratl.utils.get_peer_id(entity)
...
async def answer(
    ...
    response: str,
    *,
    reply_markup: typing.Optional[astraReplyMarkup] = None,
    **kwargs,
) -> typing.Union[InlineCall, InlineMessage, Message]:
    """
    Use this to give the response to a command
    :param message: Message to answer to. Can be a tl message or astra inline object
    ...
    parse_mode = astratl.utils.sanitize_parse_mode(
        kwargs.pop(
            "parse_mode",
    ...
    if isinstance(response, str) and not kwargs.pop("asfile", False):
        ...
        if len(text) >= 4096 and not hasattr(message, "astra_grepped"):
            try:
...
async def asset_channel(
    ...
    *,
    ...
    # legacy astra / hikka chats conversion to astra
    if title.startswith("astra-"):
        title = title.replace("astra-", "astra-")
    ...
    if _folder:
        ...
        try:
            folder = next(folder for folder in folders if folder.title == "astra")
        except Exception:
            folder = None
...
def get_commit_url() -> str:
    """
    Get current astra git commit url
    :return: Git commit url
    """
    try:
        hash_ = get_git_hash()
        return f'<a href="https://github.com/lovpq/Astra/commit/{hash_}">#{hash_[:7]}</a>'
    except Exception:
        return "Unknown"
...
def validate_html(html: str) -> str:
    ...
    :return: Valid HTML
    """
    text, entities = astratl.extensions.html.parse(html)
    return astratl.extensions.html.unparse(escape_html(text), entities)
...
def get_git_info() -> typing.Tuple[str, str]:
    ...
    return (
        hash_,
        f"https://github.com/lovpq/Astra/commit/{hash_}" if hash_ else "",
    )
