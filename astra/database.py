...
from astratl.errors.rpcerrorlist import ChannelsTooMuchError
from astratl.tl.types import Message, User
    ...
    async def init(self):
        ...
        try:
            self._assets, _ = await utils.asset_channel(
                self._client,
                "astra-assets",
                "🌆 Your astra assets will be stored here",
                archive=True,
                avatar="https://raw.githubusercontent.com/lovpq/astra/master/assets/astra-assets.png"
            )
    ...
    def read(self):
        ...
        try:
            ...
            if re.search(r'"(hikka\.)(\S+\":)', db):
                ...
                db = re.sub(r'(hikka\.)(\S+\":)', lambda m: 'astra.' + m.group(2), db)
