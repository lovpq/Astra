...
from astratl.network.connection import (
    ConnectionTcpFull,
...
class astra:
    ...
    def _read_sessions(self):
        ...
        self.sessions += [
            ...
            for session in filter(
                lambda f: f.startswith("astra-") or f.startswith("hikka-") and f.endswith(".session"),
                os.listdir(BASE_DIR),
            )
        ]
    ...
    async def save_client_session(
        ...
        *,
        ...
        client.astra_db = database.Database(client)
        await client.astra_db.init()
    ...
    async def amain_wrapper(self, client: CustomTelegramClient):
        ...
        async with client:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get("https://raw.githubusercontent.com/lovpq/astra/master/allowed_ids.txt") as response:
    ...
    async def _badge(self, client: CustomTelegramClient):
        ...
        try:
            ...
            logo = (
                "     ___            __                   \n"
                r"   /   |   _____  / /_   _____  ____ _""\n"
                r"  / /| |  / ___/ / __/  / ___/ / __ `/""\n"
                r" / ___ | (__  ) / /_   / /    / /_/ / ""\n"
                r"/_/  |_|/____/  \__/  /_/     \__,_/  ""\n"
                r"                                      ""\n\n"
                f"• Build: {build[:7]}\n"
            ...
            logging.debug(
                ...
                client.tg_id,
                client.astra_db.get(__name__, "command_prefix", False) or ".",
            )
        except Exception:
